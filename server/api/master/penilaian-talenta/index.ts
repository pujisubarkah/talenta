...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'POST') {
    ...
    const [inserted] = await db.insert(masterKompetensiItems).values({ key, label, bobot }).returning()
  ...
  if (method === 'PUT' || method === 'PATCH') {
    ...
    const [updated] = await db.update(masterKompetensiItems).set({ key, label, bobot, updated_at: new Date() }).where(eq(masterKompetensiItems.id, id)).returning()
    return { data: updated }
  }
