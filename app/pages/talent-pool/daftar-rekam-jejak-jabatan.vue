...
const rekamList = ref<RekamItem[]>([])

async function fetchRekam() {
	const res = await fetch('/api/master/rekam-jabatan')
	const json = await res.json()
	rekamList.value = Array.isArray(json) ? (json as RekamItem[]) : []
}
onMounted(fetchRekam)
...
async function addRekam() {
	await fetch('/api/master/rekam-jabatan', {
		method: 'POST',
...
function startEdit(item: RekamItem) {
	editingId.value = item.id
	editLabel.value = item.label
	editBobot.value = item.bobot
	editOptions.value = item.options
...
async function removeRekam(id: number) {
	await fetch('/api/master/rekam-jabatan', {
		method: 'DELETE',
		headers: { 'Content-Type': 'application/json' },
...
<template>
	<div class="p-6 max-w-3xl mx-auto">
		<h1 class="text-xl font-bold mb-4">Daftar Kriteria Rekam Jejak Jabatan</h1>
		...
		<table class="min-w-full divide-y divide-slate-200 text-sm mb-4">
			...
			<tbody class="divide-y divide-slate-100 bg-white text-slate-700">
				<tr v-for="(item, idx) in rekamList" :key="item.id">
						...
						<ul>
							...
							>
								<span class="font-semibold">
									{{ String.fromCharCode(97 + idx) }}. {{ opt.label }}
					...
					<td class="px-4 py-3 text-center">
						<div class="inline-flex gap-2">
							...
							<button @click="removeRekam(item.id)" class="px-2 py-1 text-xs rounded bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 flex items-center gap-1">
		...
		<div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative animate-fadein">
				...
				<h2 class="text-lg font-bold mb-4">Edit Kriteria Rekam Jejak</h2>
		...
		<div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative animate-fadein">
				...
				<h2 class="text-lg font-bold mb-4">Tambah Kriteria Rekam Jejak</h2>

				<form @submit.prevent="addRekam" class="space-y-4">
					<div>
						<label class="block text-sm font-medium mb-1">Label</label>
					...
					<div>
						...
						<ul>
							...
							>
								...
								<button type="button" @click="removeOption(idx)" class="text-xs text-red-500 hover:underline">
									Hapus
								</button>
							</li>
						</ul>
