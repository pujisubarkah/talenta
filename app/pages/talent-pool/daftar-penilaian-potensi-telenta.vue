...
async function fetchPotensi() {
	const res = await fetch('/api/master/potensi-talenta')
	const json = await res.json()
	potensiList.value = Array.isArray(json.data) ? json.data : []
}

onMounted(fetchPotensi)
...
async function removePotensi(id: number) {
	await fetch('/api/master/potensi-talenta', {
		method: 'DELETE',
		headers: { 'Content-Type': 'application/json' },
...
<template>
	<div class="p-6 max-w-2xl mx-auto">
		<h1 class="text-xl font-bold mb-4">Daftar Penilaian Potensi Talenta & Bobot</h1>
		...
		<table class="min-w-full divide-y divide-slate-200 text-sm mb-4">
			...
			<tbody class="divide-y divide-slate-100 bg-white text-slate-700">
				<tr v-for="(item, idx) in potensiList" :key="item.id">
					...
					<td class="px-4 py-3 text-center">
						<div class="inline-flex gap-2">
							...
							<button @click="removePotensi(item.id)" class="px-2 py-1 text-xs rounded bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 flex items-center gap-1">
		...
		<div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm relative">
				...
				<h2 class="text-lg font-bold mb-4">Edit Penilaian Potensi Talenta</h2>
		...
		<div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm relative">
				...
				<h2 class="text-lg font-bold mb-4">Tambah Penilaian Potensi Talenta</h2>
				<form @submit.prevent="addPotensi" class="space-y-4">
					<div>
						<label class="block text-sm font-medium mb-1">Nama Penilaian</label>
